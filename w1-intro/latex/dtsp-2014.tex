...
\begin{frame}\frametitle{Overview}\small
  ...
  \begin{itemize}
    ...
    \item {\em Transforms}:  discrete-time Fourier transform 
    \item {\em Frequency Response \& Filters}: transfer functions 
  \end{itemize}
  \end{block}
...
\begin{frame}\frametitle{Example}\small
  ...
  \begin{align}
    ...
    \nonumber
  \end{align}
  \uncover<2->{If $y[n] = h[n]\star x[n]$, then }
  \begin{align}
    ...
    \nonumber
  \end{align}
  \uncover<5->{For the simple example of an ideal delay we have $\angle H(\e^{j\omega_0} = -\omega_0 n_0$, then 
  \begin{align}
...
\begin{frame}\frametitle{Example}\small
   ...
   Let us determine the impulse response of an {\em ideal} low-pass filter. The frequency response is given by: 
   \begin{align}
...
\begin{frame}\frametitle{Frequency Differentiation}\small
  ...
  \begin{align}
    \frac{\d X(\e^{j\omega})}{\d \omega} \uncover<2->{= \frac{\d}{\d\omega}\left\{\sum_{n=-\infty}^{\infty} x[n] \e^{-j\omega n}\right\} }
    \uncover<3->{= \sum_{n=-\infty}^{\infty} x[n] \frac{\d}{\d\omega}\left\{\e^{-j\omega n}\right\}}
    \uncover<4->{= -j \sum_{n=-\infty}^{\infty} n x[n] \e^{-j\omega n} }\nonumber
  \end{align}
...
\begin{frame}\frametitle{Discrete-Time Fourier Transforms using Tables}\small
  ...
  \uncover<5->{
    {\noindent\bf\color{blue!50!black} Does this look unfamiliar?} Read up on partial fraction expansion. 
  }
\end{frame}
...
\begin{frame}\frametitle{Example}\small
  ...
  \begin{center}
    \uncover<2->{\includegraphics[height=.4\textheight]{hn_example.pdf}}
  \end{center}
\end{frame}
  ...
  \begin{block}{The Sampling Theorem}
    \begin{itemize}
      \item A bandlimited signal can be reconstructed exactly from samples taken with sampling frequency
        ...
        \end{align}
      \item<2-> {\color{blue}Question}: If I sampled a signal $x(t)$ which is a cosine with $f = 34,723,487$Hz at a rate of $f_s = 1,234$Hz, where will it show up in the spectrum after sampling? Are we seeing folding?
    \end{itemize}
  \end{block}
\end{frame}
